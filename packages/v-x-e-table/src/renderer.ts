...
import { getCellValue, setCellValue } from '../../table/src/utils';
...
function getCellEditFilterProps(renderOpts: any, params: any, value: any, defaultProps?: any) {
  return XEUtils.assign({}, defaultCompProps, defaultProps, renderOpts.props, {
    [componentDefaultModelProp]: value,
  });
}
...
function defaultButtonsEditRender(renderOpts: any, params: any) {
  return renderOpts.children.map(
    (childRenderOpts: any) => defaultButtonEditRender(childRenderOpts, params)[0],
  );
}
...
function defaultButtonsItemRender(renderOpts: any, params: any) {
  return renderOpts.children.map(
    (childRenderOpts: any) => defaultButtonItemRender(childRenderOpts, params)[0],
  );
}
...
function defaultFormItemRadioAndCheckboxRender(renderOpts: any, params: any) {
  ...
  if (options) {
    return [
      h(
        resolveComponent(`${compName}-group`) as ComponentOptions,
        {
          ...getComponentFormItemProps(renderOpts, params, itemValue),
          ...getItemOns(renderOpts, params),
        },
    ...
    renderItemContent(renderOpts: any, params: any) {
      return [
        h(
          ...
          renderOpts.optionGroups
            ? renderNativeOptgroups(renderOpts, params, renderNativeFormOptions)
            : renderNativeFormOptions(renderOpts.options, renderOpts, params),
        ),
      ];
    },
