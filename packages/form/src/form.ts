...
import { createItem, handleFieldOrItem, isHiddenItem, isActivetem } from './utils';
...
export default defineComponent({
  ...
  props: {
    ...
    span: {
      ...
      default: () => GlobalConfig.form.span,
    },
    align: {
    ...
    titleWidth: {
      ...
      default: () => GlobalConfig.form.titleWidth,
    },
    titleColon: {
    ...
    customLayout: {
      ...
      default: () => GlobalConfig.form.customLayout,
    },
  },
  emits: [
  ...
  setup(props, context) {
    ...
    const loadItem = (list: VxeFormPropTypes.Items) => {
      if (list.length) {
        reactData.staticItems = XEUtils.mapTree(list, item => createItem($xeform, item), {
          children: 'children',
        });
      }
      return nextTick();
    ...
    const getItemByField = (field: string) => {
      const rest = XEUtils.findTree(reactData.formItems, item => item.field === field, {
        children: 'children',
      });
    ...
    const validItemRules = (
      ...
      val?: any,
    ): Promise<VxeFormDefines.ValidateErrorMapParams | undefined> => {
    ...
    const beginValidate = (
      itemList: VxeFormDefines.ItemInfo[],
      type?: string,
      callback?: any,
    ): Promise<any> => {
      const { data, rules: formRules } = props;
    ...
    const validateField = (
      ...
      return beginValidate(
        fields.map(field => handleFieldOrItem($xeform, field) as VxeFormDefines.ItemInfo),
        '',
        callback,
      );
    };
          ...
          class: [
            ...
            className
              ? XEUtils.isFunction(className)
                ? className({ items: formItems, data, $form: $xeform })
                : className
              : '',
            {
              [`size--${vSize}`]: vSize,
