  ...
  setup(props, context) {
    ...
    const restorePosStorage = () => {
      ...
      if (id && remember && storage) {
        ...
        if (posStorage) {
          const boxElem = getBox();
          const [left, top, width, height, zoomLeft, zoomTop, zoomWidth, zoomHeight] =
            posStorage.split(',');
          if (left) {
    ...
    const savePosStorage = () => {
      ...
      if (id && remember && storage) {
        ...
        posStorageMap[id] = (
          [boxElem.style.left, boxElem.style.top, boxElem.style.width, boxElem.style.height] as (
            | string
            | number
          )[]
        )
          .concat(
            zoomLocat ? [zoomLocat.left, zoomLocat.top, zoomLocat.width, zoomLocat.height] : []
          )
    ...
    const boxMousedownEvent = () => {
      ...
      if (
        allActivedModals.some(
          comp => comp.reactData.visible && comp.reactData.modalZindex > modalZindex
        )
      ) {
        updateZindex();
      }
        ...
        rightVNs.push(
          h('i', {
            class: [
              'vxe-modal--zoom-btn',
              'trigger--btn',
              zoomLocat ? GlobalConfig.icon.MODAL_ZOOM_OUT : GlobalConfig.icon.MODAL_ZOOM_IN,
            ],
    ...
    const renderHeaders = () => {
      const { slots: propSlots = {}, showZoom, draggable, showHeader } = props;
      const isMsg = computeIsMsg.value;
      ...
      if (showHeader) {
        ...
        headVNs.push(
          h(
            ...
            headerSlot
              ? !reactData.inited || (props.destroyOnClose && !reactData.visible)
                ? []
            ...
            [
              h('i', {
                class: [
                  ...
                  props.iconStatus || GlobalConfig.icon[`MODAL_${status}`.toLocaleUpperCase()],
                ],
              }),
            ]
          )
    ...
    const renderVN = () => {
      const { className, type, animat, loading, status, lockScroll, lockView, mask, resize } =
        props;
