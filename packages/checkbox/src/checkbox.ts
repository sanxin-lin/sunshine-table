...
import {
  VxeCheckboxConstructor,
  VxeCheckboxGroupConstructor,
  VxeCheckboxEmits,
  VxeCheckboxGroupPrivateMethods,
  CheckboxMethods,
...
export default defineComponent({
  ...
  emits: ['update:modelValue', 'change'] as VxeCheckboxEmits,
  setup(props, context) {
    const { slots, emit } = context;
    const $xeform = inject<(VxeFormConstructor & VxeFormPrivateMethods) | null>('$xeform', null);
    const $xeformiteminfo = inject<VxeFormDefines.ProvideItemInfo | null>('$xeformiteminfo', null);
    const $xecheckboxgroup = inject(
      '$xecheckboxgroup',
      null as (VxeCheckboxGroupConstructor & VxeCheckboxGroupPrivateMethods) | null
    );
